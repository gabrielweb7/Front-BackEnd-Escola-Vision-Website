...
$.fn.menuParalax7 = function( options ) {
  ...
  var checkMenuActive = function() {
    ...
    /* Reset Active Menus  */
    $(menuScrollAutoFixA).parent().removeClass('active');
    ...
    for (var item in menuList) {
      ...
      var tamanhoMenuElement = $("header .topo").outerHeight();

      debug(item +" = offsetTop:"+offsetTop+ " sectionHeight:"+ sectionHeight + "  distanceLimited: "+ distanceLimited+" tamanhoMenuElement:"+tamanhoMenuElement);

      if(getPosScroll() >= (offsetTop-tamanhoMenuElement) && getPosScroll() < (distanceLimited-tamanhoMenuElement)) {
   ...
   /* console.log('achouMenu: '+achouMenu);*/
  ...
  menuScrollAutoFixA.on('click', function(e){
    ...
    /* caso for responsive ele subir menu */
    if($(window).width() < 576) { 
        $("nav.menu").stop().slideUp(500);
        ...
        setTimeout(function() { 
          ...
          if($(settings.mySelectorSection+'.'+ hash).length > 0) {
            ...
            var goTo =  $(settings.mySelectorSection+'.'+ hash).position().top-tamanhoMenuElement;
